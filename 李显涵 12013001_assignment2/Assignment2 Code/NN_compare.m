...
large = [2,5,10,20,30,50,70,100];
acc_compare = [];
auc_compare = [];

for t = 1:length(large)
...
% 定义前馈网络
% network = feedforwardnet([4,8,4],'traingd');

network = feedforwardnet(large(t),'traingd');
...
% disp(strcat('Average accuracy after 5-fold cross-validation: ',num2str(accuracy),'%'));
acc_compare = [acc_compare, accuracy_final];
...
figure(2);
plot(large,acc_compare);
title('hidden layer size-accurancy');
xlabel('hidden layer size');
ylabel('accurancy(%)');


figure(3);
plot(large,auc_compare);
title('hidden layer size-AUC');
xlabel('hidden layer size');
ylabel('AUC');